...
describe('Card Component', () => {
  ...
  describe('Rendering', () => {
    test('should render card with basic information', () => {
      ...
      expect(screen.getByText('UCLA ➡ LAX')).toBeInTheDocument();
  ...
  describe('Owner Features', () => {
    ...
    test('should open delete confirmation modal when delete button clicked', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/permanently delete this ride group/i)).toBeInTheDocument();
      });
    });
  ...
  describe('Join/Leave Ride Functionality', () => {
    ...
    test('should call leaveRide API when leave is confirmed', async () => {
      ...
      await waitFor(() => {
        const leaveButton = screen.getByRole('button', { name: /Cancel Request/i });
  ...
  describe('Pending Requests (Owner)', () => {
    test('should fetch pending requests when owner opens Requests tab', async () => {
      ...
      const joinButton = screen.getByRole('button', { name: /View Ride/i });
      fireEvent.click(joinButton);
    ...
    test('should approve pending request', async () => {
      ...
      await waitFor(() => {
        expect(ridesApi.manageRequest).toHaveBeenCalledWith('ride-123', 5, 'approve');
      });
    });
    ...
    test('should reject pending request', async () => {
      ...
      await waitFor(() => {
        expect(ridesApi.manageRequest).toHaveBeenCalledWith('ride-123', 5, 'reject');
      });
    });
  });
  ...
  describe('Transfer Ownership', () => {
    test('should transfer ownership when Transfer Ownership is clicked', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: /Riders/i })).toBeInTheDocument();
      });
      ...
      await waitFor(() => {
        const transferButton = screen.getByRole('button', { name: /Make Owner/i });
        fireEvent.click(transferButton);
      });

      await waitFor(() => {
        expect(ridesApi.transferOwnership).toHaveBeenCalledWith('ride-123', 3);
        expect(defaultProps.onTransferOwnership).toHaveBeenCalledWith('ride-123', 3);
      });
    });
  });
  ...
  describe('Kick Member', () => {
    test('should kick member when owner clicks Kick button', async () => {
      ...
      await waitFor(() => {
        expect(ridesApi.kickMember).toHaveBeenCalledWith('ride-123', 3);
      });
    });
  });
  ...
  describe('View Profile', () => {
    test('should navigate to profile when View Profile is clicked', async () => {
      ...
      const profileLink = screen.getByText('Alice Wonder');
